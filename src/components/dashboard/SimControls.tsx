  ...
  return (
    <div className="hidden sm:flex items-center gap-3 text-[11px] font-mono text-slate-400">
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-2 py-1 rounded-sm border text-[10px] font-bold uppercase tracking-wider transition-colors ${
          isSimRunning
            ? 'bg-amber-950/40 border-amber-900/50 text-amber-400 hover:bg-amber-900/40'
            : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700 hover:text-slate-200'
        }`}
      >
        {isSimRunning ? <Pause size={10} /> : <Play size={10} />}
